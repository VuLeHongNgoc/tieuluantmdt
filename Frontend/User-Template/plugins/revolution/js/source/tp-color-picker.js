...
(function ($) {
  ...
  function init() {
    ...
    colorBtn.data(
      'state',
      colorSection.find('.rev-cpicker-color').eq(0).attr('data-color') ||
        '#ffffff'
    );
    gradBtn.data(
      'state',
      gradSection.find('.rev-cpicker-color').eq(0).attr('data-color') ||
        'linear-gradient(0deg, rgba(255, 255, 255, 1) 0%, rgba(0, 0, 0, 1) 100%)'
    );
    ...
    $('.rev-cpicker-main-btn').on('click.revcpicker', function () {
      ...
      if (this.id.search('gradient') === -1) {
        ...
        if (!gradViewed)
          gradCore.children('.rev-cpicker-color').not('.blank').eq(0).click();
      }
    ...
    $('.rev-cpicker-point-location').on(
      'keyup.revcpicker focusout.revcpicker',
      function (e) {
        if (!curPoint) return;
