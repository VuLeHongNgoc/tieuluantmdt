...
var Cufon = (function () {
  ...
  var DOM = (api.DOM = {
    ready: (function () {
      ...
      if (!window.opera && document.readyState)
        (function () {
          readyStatus[document.readyState]
            ? perform()
  ...
  var CSS = (api.CSS = {
    ...
    gradient: cached(function (value) {
      var gradient = {
        ...
        colors = value
          .substr(value.indexOf('('))
          .match(/([\d.]+=)?(#[a-f0-9]+|[a-z]+\(.*?\)|[a-z]+)/gi);
  ...
  CSS.ready = (function () {
          ...
          search: for (
            ...
            switch (rule.type) {
              case 2: // @charset
                break;
              case 3: // @import
                if (!isSheetReady(rule.styleSheet, rule.media.mediaText))
                  return false;
                break;
              default:
    ...
    function allStylesLoaded() {
      ...
      for (i = 0; (el = links[i]); ++i) {
        if (el.rel.toLowerCase() == 'stylesheet' && !isContainerReady(el))
          return false;
      }
  ...
  function Font(data) {
    ...
    this.applyLigatures = function (text, ligatures) {
      ...
      for (
        var i = 0, ligatureConfig;
        i < ligatureCache.length && !ligatureConfig;
        i++
      )
      ...
      if (!ligatureConfig) {
        ...
        ligatureCache.push(
          (ligatureConfig = {
            ...
            regexp:
              regexpText.length > 0
                ? regexpCache[regexpText] ||
                  (regexpCache[regexpText] = new RegExp(regexpText, 'g'))
                : null,
          })
        );
      }
  ...
  function updateShy(context) {
    ...
    for (i = 0; (shy = shys[i]); ++i) {
      ...
      prev =
        next.nodeName.toLowerCase() == TAG_GLUE ? glue : shy.previousSibling;
  ...
  api.replace = function (elements, options, ignoreHistory) {
    ...
    if (typeof options.ignoreClass == 'string') {
      options.ignoreClass = new RegExp(
        '(?:^|\\s)(?:' + options.ignoreClass.replace(/\s+/g, '|') + ')(?:\\s|$)'
      );
    }
  ...
  (function () {
    ...
    function getFontSizeInPixels(el, value) {
      return getSizeInPixels(
        el,
        /(?:em|ex|%)$|^[a-z-]+$/i.test(value) ? '1em' : value
      );
    }
    ...
    function getSpacingValue(el, style, size, property) {
      ...
      if (isNaN(value)) {
        ...
        style[key] = value =
          value == 'normal'
            ? 0
            : ~~size.convertFrom(getSizeInPixels(el, value));
      }
      return value;
    }
    ...
    return function (font, text, style, options, node, el, hasNext) {
      ...
      var stretchFactor =
        roundingFactor * Cufon.CSS.fontStretch(style.get('fontStretch'));
      ...
      while ((chr = chars[++i])) {
        ...
        shape.path =
          (glyph.d ? 'm' + glyph.d + 'xe' : '') + 'm' + coordOrigin + stretch;
      ...
      wStyle.width = Math.max(
        Math.ceil(size.convert(width * stretchFactor)),
        0
      );
  ...
  (function () {
    ...
    return function (font, text, style, options, node, el) {
      ...
      var canvasWidth = Math.ceil(
        size.convert(stretchedWidth + expandRight - expandLeft)
      );
      var canvasHeight = Math.ceil(
        size.convert(viewBox.height - expandTop + expandBottom)
      );
      ...
      var wrapperWidth =
        Math.max(Math.ceil(size.convert(stretchedWidth)), 0) + 'px';
